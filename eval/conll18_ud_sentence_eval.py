...
def load_conllu(file):
    ...
    while True:
        ...
        if not line:
            ...
            #on new sentence append current UD representation to sentence UD list
            sentence_udrepresentations.append(ud)
        ...
        # Basic tokens/words
            ...
            #print ("word_id" + str(int(columns[ID])))
            try:
    ...
    return sentence_udrepresentations
    # return ud
...
def evaluate_wrapper(args):
    ...
    #loop over individual sentences in gold and system outputs and get scores
    for g, s in zip(gold_ud, system_ud):
        sentence_evaluations.append(evaluate(g, s))

    return sentence_evaluations
...
def main():
    ...
    for sentence in evaluation:
        ...
        if not args.verbose and not args.counts:
            print("   {:>4}   {:>3}    0   {:>6.2f}  {:>6.2f}    {:>3}      {:>3}          {:>3}    0 0 0 0".format(sentenceID, sentence["LAS"].gold_total, 100*sentence["UAS"].f1, 100*sentence["LAS"].f1, sentence["UAS"].correct, sentence["LAS"].correct, sentence["LAS"].gold_total))
            sentenceID += 1
            total += sentence["LAS"].gold_total
            las_correct += sentence["LAS"].correct
            uas_correct += sentence["UAS"].correct

    print ("\n\nLabeled attachment score: {:.2f}".format(100*las_correct/total))
    print ("Unlabeled attachment score: {:.2f}".format(100*uas_correct/total))
    print ("============================================================================")
