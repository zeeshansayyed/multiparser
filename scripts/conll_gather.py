...
def get_gold_file(lang, task, treebank, split):
    ...
    if task in ('ud', 'sud'):
        ...
        if lang == 'arabic':
            ...
            lang = lang.title()
        gold_file = data_root / 'spmrl' / lang.lower() / f'{split}.{lang}.gold.conll'
    else:
...
def gather_baseline(lang, tasks, embed_extension, exp_type, seed, split):
    baseline_result = []
    for task in tasks:
        ...
        UAS, LAS = evaluate(lang, conll_file)
        baseline_result += [UAS, LAS]
    return baseline_result


def gather_multitask(lang, tasks, treebank, embed_extension, exp_type, loss, mlp, seed, split):
    multitask_result = []
    for task in tasks:
...
def gather(lang, tasks, treebank, embed_extension, exp_types, losses, mlps, seed,
    ...
    if not embed_extension:
        embed_extension = treebank

    baseline_results = {}
    multitask_results = defaultdict(list)

    for exp_type in exp_types:    
        res = gather_baseline(lang, tasks, embed_extension, exp_type, seed, split)
        baseline_results[exp_type] = res

        for loss in losses:
            for mlp in mlps:
                res = gather_multitask(lang, tasks, treebank, embed_extension, exp_type, loss, mlp, seed, split)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--treebank', required='True', help="Name of treebank")
