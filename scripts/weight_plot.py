...
# exp_files = exp_files1 + exp_files2 + exp_files3 + exp_files4 + exp_files5
# exp_files = ['de_full_none']
exp_files = ['it_4k_isdt-tw_dev']

for exp_file in exp_files:
    data = pd.read_csv(f'results/domain/{exp_file}.csv', index_col=0)
    ...
    t1, t2 = list(data.loc[0.50].values)
    plt.axvline(10, 0, max_val, color='crimson', ls=':')
    plt.axhline(t1, 0, 20, color='blue', ls=':')
    plt.axhline(t2, 0, 20, color='orange', ls=':')

    plt.legend(fontsize='x-large', title_fontsize='40', loc='lower center')
    plt.xlabel(f"Weights for {data.columns[0]}", fontsize=12)
    plt.ylabel(f"LAS", fontsize=12)
    plt.tight_layout()
    plt.savefig(f'results/domain/{exp_file}.png')